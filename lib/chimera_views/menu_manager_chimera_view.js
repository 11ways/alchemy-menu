...
alchemy.create('ChimeraView', function MenuManagerChimeraView() {
	...
	/**
	 * Saves the new order of menu pieces
	 */
	this.configure_order = function configure_order(render, module, options) {

		var ordered = render.req.body.ordered,
		    tasks   = {},
		    MP      = this.getModel('MenuPiece');

		if (!render.req.body.ordered) {
			// @todo: return error
			render.res.end();
		}

		Object.each(ordered, function(nr, id) {
			...
			if (id && String(id).isObjectId()) {
				...
				tasks[id] = function(next) {
					...
					MP.save(data, function(err, result) {
						next(err, result);
					});
				};
			}
	...
	this.configure = function configure(render, module, options) {
		...
		if (data && body.createNew) {
			...
			MP.save(obj, function(err, results) {
				...
				delete render.req.body.createNew;
		...
		MP.find('first', {conditions: {_id: pieceId}}, function(err, result) {
			...
			ME.getModelGroups(type, piece, function(err, groups) {
				...
				if (render.post) {
					...
					ME.prepareSave(piece.settings, {
						...
						MP.save(piece, function(err, results) {
							...
							ME.getModelGroups(type, results[0].item, function(err, __groups) {
